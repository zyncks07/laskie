...
# ── Step 7: Write credentials to config/db.php ────────────────
step "Writing database credentials"

cat > "${APP_ROOT}/config/db.php" <<PHP
<?php
// config/db.php — generated by deploy.sh
...
try {
    ...
    if (defined('JSON_RESPONSE') && JSON_RESPONSE) {
        ...
        echo json_encode(['success' => false, 'error' => 'Database connection failed. Check config/db.php']);
        exit;
    }
    die('<div style="font-family:sans-serif;padding:2rem;color:#dc2626;"><h2>Database Connection Error</h2><p>Could not connect to the database. Please check <code>config/db.php</code> and ensure MariaDB is running.</p></div>');
}
PHP

ok "config/db.php written."
