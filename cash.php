...
$myCash = (float)$myBalance['total_received'] - (float)$myBalance['total_remitted'] - (float)$myBalance['total_expenses'];
...
<div class="cash-hero mb-3">
  <div class="row g-3 align-items-center">
    <div class="col-md-4">
      ...
      <div style="font-size:36px;font-weight:800;letter-spacing:-1px"><?=money($myCash)?></div>
      ...
      <div class="row g-2">
        <div class="col-4">
          <div style="background:rgba(255,255,255,.12);border-radius:8px;padding:12px;text-align:center">
            ...
            <div style="font-size:18px;font-weight:700"><?=money((float)$myBalance['total_received'])?></div>
          </div>
        </div>
        <div class="col-4">
          <div style="background:rgba(255,255,255,.12);border-radius:8px;padding:12px;text-align:center">
            <div style="font-size:10px;text-transform:uppercase;letter-spacing:.08em;opacity:.75;margin-bottom:4px">Total Remitted</div>
            <div style="font-size:18px;font-weight:700"><?=money((float)$myBalance['total_remitted'])?></div>
          </div>
        </div>
        <div class="col-4">
          <div style="background:rgba(255,255,255,.12);border-radius:8px;padding:12px;text-align:center">
            <div style="font-size:10px;text-transform:uppercase;letter-spacing:.08em;opacity:.75;margin-bottom:4px">Total Expenses</div>
            <div style="font-size:18px;font-weight:700"><?=money((float)$myBalance['total_expenses'])?></div>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="table-responsive">
    <table class="table">
      <thead><tr>
        ...
        <th class="text-center">Proof</th><th class="text-center no-print">Del</th>
      </tr></thead>
      <tbody id="txBody">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title"><i class="fa-solid fa-paper-plane me-2"></i>Record Remittance</h5>
...
function loadTransactions() {
  ...
  apiPost('api/cash_api.php', {action:'list_transactions', user_id:userId, month:month, year:year, type:type}, function(err, res) {
    if (!res || !res.success) {
    ...
    var typeBadge = {received:'badge-received', remitted:'badge-remitted', expense:'badge-expense'};
    var typeIcon  = {received:'fa-arrow-down',  remitted:'fa-arrow-up',    expense:'fa-minus-circle'};
    var typeLabel = {received:'Collection',      remitted:'Remitted',       expense:'Expense'};
    var amtColor  = {received:'var(--success)',  remitted:'var(--info)',    expense:'var(--danger)'};
    ...
    for (var i = 0; i < res.transactions.length; i++) {
      ...
      html += '<td class="text-center">' + delBtn + '</td>';
      html += '</tr>';
    }
...
function saveRemittance() {
  ...
  fetch('api/cash_api.php', {method:'POST', body:fd, credentials:'same-origin'})
